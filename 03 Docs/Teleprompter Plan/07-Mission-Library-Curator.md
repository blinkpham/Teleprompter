...
First read the existing `src/content/` files, the V2 adapter when it exists, and the original `image-director/SKILL.md`, `assets/quick-snippets.md`, and relevant headings in `references/prompting-strategy.md`. Read your latest `STATE.md` and accepted batch index on subsequent runs. Avoid loading all prior transcripts or research notes.
