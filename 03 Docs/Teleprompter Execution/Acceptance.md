...
| Native spotlight fallback | partially accepted | Native `View → Show Cue` opened a separate `Teleprompter Cue` window with renderer URL `localhost:5173/?surface=spotlight`; the rendered dark Cue was visible and the panel dismissed when focus moved away. |
...
| Spotlight cursor placement, resize, and direct dismissal | unproven | The separate panel appeared through the menu, but this run did not obtain stable inspectable bounds or a second independent shortcut witness. Do not infer these from placement unit tests or menu registration. |
...
The global shortcut remains a limitation despite the persisted registered flag. A menu path is a valid fallback, but it does not upgrade the shortcut result to accepted.
